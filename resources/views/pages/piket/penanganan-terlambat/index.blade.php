        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('piket.penanganan-terlambat.index') }}" method="GET">
                        ...
                        @if ($hasilPencarian->count() == 1)
                            ...
                            <form action="{{ route('piket.penanganan-terlambat.store') }}" method="POST">
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div>
                                        <x-input-label for="alasan" value="Alasan Keterlambatan" />
                                        <textarea name="alasan" id="alasan" rows="3" class="w-full border-gray-300 rounded-md shadow-sm" required>{{ old('alasan') }}</textarea>
                                    </div>
                                    <div>
                                        <x-input-label for="tindak_lanjut" value="Tindak Lanjut (Opsional)" />
                                        <textarea name="tindak_lanjut" id="tindak_lanjut" rows="2" class="w-full border-gray-300 rounded-md shadow-sm">{{ old('tindak_lanjut') }}</textarea>
                                    </div>
                                    <div class="flex justify-end">
                                        <x-primary-button type="submit">Simpan & Cetak Surat</x-primary-button>
                                    </div>
                                </div>
                            </form>
