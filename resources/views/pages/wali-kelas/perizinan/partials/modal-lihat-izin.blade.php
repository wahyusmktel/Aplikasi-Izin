<div x-data="{ show: false, item: {} }"
     @open-modal.window="if ($event.detail.name === 'lihat-izin-wali-kelas') { show = true; item = $event.detail.item; }"
    ...
    <div class="flex items-end justify-center min-h-screen px-4 text-center md:items-center sm:block sm:p-0">
        ...
        <div x-show="show"
            ...
            <div class="mt-4 space-y-2">
                ...
                <p><strong>Tanggal Izin:</strong> <span x-text="new Date(item.tanggal_izin).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })"></span></p>
                <div>
                ...
                <template x-if="item.dokumen_pendukung">
                    <p><strong>Dokumen Pendukung:</strong> <a :href="'/storage/' + item.dokumen_pendukung.replace('public/', '')" target="_blank" class="text-blue-600 hover:underline">Lihat Dokumen</a></p>
                </template>
                <template x-if="item.status === 'ditolak'">
                    <div>
                        <p><strong>Alasan Penolakan:</strong></p>
                        <p class="text-red-600" x-text="item.alasan_penolakan"></p>
                    </div>
                </template>
            </div>
